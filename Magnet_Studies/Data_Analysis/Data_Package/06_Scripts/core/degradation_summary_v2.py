...
def build_temperature_lookup():
    ...
    (pair-level) in addition to An/As (pair) files. This fixes
    date matching for pre-deployment measurements where the H-slot and A-pair
    ...
    for f in os.listdir(pair_helm_dir):
        ...
        # Source 1: An/As pair Teslameter
...
def compute_h_plate_degradation(pair_arrangements, temp_lookup):
    ...
    for f in sorted(os.listdir(helm_dir)):
        ...
        # Per-session % changes (for time-series analysis)
        date_pcts = [(dt, (h_corr - baseline_mean) / baseline_mean * 100.0)
                     for dt, h_raw, h_corr, dh in tunnel_corr]
...
def write_report(y_res, h_res, tesla_res, y_excl, h_excl):
    ...
    L.append("**Treatment**: v2 uses **only temperature-corrected readings** for both "
             ...
             "Teslameter files (not just An/As pair files), which recovers "
             "34 additional temperature-matched baseline readings from the Nov 2024 "
             "campaign where H-slot and A-pair measurements were taken on different days.")
    L.append("")
