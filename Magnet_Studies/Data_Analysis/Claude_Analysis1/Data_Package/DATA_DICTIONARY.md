...
| environment | string | — | "tunnel" or "lab" |
| baseline_mean_mWC | float | mWC | Mean pre-deployment Helmholtz reading, temperature-corrected to 20 C. For lab plates, single baseline reading (temp-corrected). |
| latest_mWC | float | mWC | Latest post-deployment reading, temperature-corrected. |
| pct_change | float | % | (latest - baseline) / baseline * 100. Negative = demagnetization. |
| baseline_sem_pct | float | % | Standard error of the mean baseline, as % of baseline. 0.0 for lab (single baseline). |
...
Machine-readable summary of all key results (24 rows).
...
| uncertainty | float | Statistical uncertainty (0 if not applicable); for Spearman rows, this is the p-value |
| significance | string | Statistical significance (e.g., "7.6 sigma" or "p=0.0338") |
| notes | string | Description and sample size |

**Metrics included**: tunnel differential, lab control differential, tunnel-lab excess,
4 per-material means, 2 gain systematic values, 4 temperature coefficients,
differential temperature sensitivity, 3 systematic uncertainties (temp, alpha, total),
combined uncertainty, and 6 Spearman dose-degradation correlations (gamma and neutron
vs NdFeB mean, SmCo mean, and differential).

### verify_results.py (06_Scripts/)
Standalone verification script that reads only data package CSVs and reproduces
all 21 headline checks. Run with: `cd Data_Package/ && python3 06_Scripts/verify_results.py`
