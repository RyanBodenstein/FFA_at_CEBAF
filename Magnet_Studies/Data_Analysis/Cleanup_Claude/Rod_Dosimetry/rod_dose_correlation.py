...
def plot_R1_side_by_side(merged, gain_syst):
    ...
    datasets = [
        ('OSL Body Dose (mrem)', 'osl_body_mrem', 'osl_is_lower_bound', 1.0, False),
        ('AIdata Photon Dose (Gy)', 'ai_photon_gy', None, 1.0, True),
    ]

    for ax, (xlabel, dose_key, lb_key, scale, show_xerr) in zip(axes, datasets):
        ...
        for m in merged:
            ...
            if np.isnan(y_err):
                y_err = 0
            x_err = m.get('ai_sigma_photon_gy', 0) if show_xerr else 0
