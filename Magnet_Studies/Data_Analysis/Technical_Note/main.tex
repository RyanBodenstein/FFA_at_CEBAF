...
{\large R.M.\ Bodenstein, C. Decker, K.\,E. Deitrick, B.\,R. Gamage, J.\,F. Gubeli, D. Hamlette,\\ M. Janak, K. Jordan, S. Lee, M. Mason, C. Matthews, J. Meyers, B. Mosbrucker,\\ I. Neththikumara,  E. Nissen, V. Okey-Ejiowhor, A. Ramirez, D. Thomas, J. Samari,\\ S. Shriner, M. Smith, N. Wilson, and External Collaborators}\\[2.5cm]
...
\begin{itemize}
    \item High-dose studies \cite{Simos2018,Bizen2016,Alderman}
...
\begin{itemize}
    ...
    \item July 2025: Magnets deployed in the \CEBAF~tunnel.
    \item July 2025--January 2026: Post-deployment measurement
    campaigns (periodic removal for measurement, then return to tunnel).
    \item Total exposure: $\sim$18 months of parasitic radiation
...
same device. The original probe broke and was replaced between
...
\begin{enumerate}
    ...
    \item \emph{Robust}: The signal exceeds $5\sigma$ for \emph{any}
...
\begin{itemize}
    ...
    thermal neutrons, $Q = 2.5$ (ICRP~103 continuous $w_R$ function; the 10~CFR~20 step function gives $w_R = 5$). Converting to absorbed dose (Gy)
...
\begin{enumerate}
    ...
    artifact: the measurement system is unbiased.
...
\begin{figure}[H]
    ...
    significant positive correlation ($p = 0.011$). Error bars: $x =$ dose $\sigma$, $y = \pm
    1$~SEM.}
    \label{fig:R1}
\end{figure}
...
\begin{itemize}
    \item Alderman et al.~\cite{Alderman} irradiated NdFeB with pure
    gamma radiation (\SI{60}{Co} equivalent) up to \SI{7}{MGy} (700~Mrad) and
    observed no measurable demagnetization. This is the highest-dose
    gamma-only null in the literature.
    \item Our maximum gamma dose (\SI{23451}{Gy} at Y-22) is
    $\sim$300$\times$ below the Alderman null threshold.
...
\begin{itemize}
    \item Thermal neutrons damage magnets primarily via nuclear
    capture reactions, especially ${}^{10}$B(n,$\alpha$)${}^{7}$Li ($\sigma =
    \SI{3840}{barns}$\footnote{1~barn $= 10^{-24}$~cm$^2 = 10^{-28}$~m$^2$.}, $Q = \SI{2.3}{MeV}$). The alpha particle and lithium
    ion create localized ionization tracks that can disrupt grain boundary
    phases.
    \item Fast neutrons damage via displacement cascades: the primary
...
neutrons, the calculation separates the two components reported by the CR-39
dosimeters. For thermal neutrons, $h^*(10) = \SI{7.6}{pSv \cdot cm^2}$; for
...
disrupt grain boundary phases. SmCo contains no boron, making it
...
Yet SmCo shows zero degradation. This proves that neutron capture
...
\begin{description}[style=nextline]
    ...
    synchrotron radiation) up to \SI{7}{MGy} and observed no
